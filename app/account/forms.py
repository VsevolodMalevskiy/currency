...
class UserSignUpForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        user = super().save(commit=False)  # объект не сохраняется в базу данных, так как commit=False - для дополнений
        password = self.cleaned_data['password1']
        # user.password = password -> WRONG
        user.is_active = False
        user.username = uuid.uuid4()  # генератор
        user.set_password(password)  # меняется объект User - добавляется password
        user.save()
